...
//					popup.showAsBalloon(3476, mapObject, tempMarker);
	...
	(function($topStories){
		...
		$(window).scroll( function() {
			if (( $(window).scrollTop() >= $(document).height() - $(window).height() - 10 ) && imageContainer.is(':visible')) { 
				if( pageNum < maxPages ){
					pageNum++;
					loadItems();
				}
	...
	$TN.initAndLoadHomePage = function(){
		...
		$('#searchCategoryArrow').click(function(){
			var jqDropDown = $('#searchCategoryDropDown');
			
			if( !dropDownOpen ){
	    		var jqElem = $(this);
	    		var refOffset = jqElem.offset();
	    		var refWidth = jqElem.outerWidth();
	    		var refHeight = jqElem.outerHeight();
	    		
	    		var dropdownWidth = jqDropDown.outerWidth();
	    		jqDropDown.css('top', refOffset.top + refHeight).css('left', refOffset.left + refWidth - dropdownWidth ).show();
	    		dropDownOpen = true;
	    		jqDropDown.focus();
			}
			else {
