if (!TN) var TN= {};
if(!TN.newspaperView) TN.newspaperView = {};
...
(function($newspaperView){
    ...
    function getStoryHtml(storyObj){
		...
		var storyHtml = '<div id="story_' + storyObj.buzzId + '" class="storyNPBlock" onclick="TN.newspaperView.addWidget(this);" story-id="' + storyObj.buzzId +'">\
	...
	function loadNewspaper(npId){
		function showNewspaper(json){
			...
			if( !!json && !!json[0] ){
				...
				var NPTemplate = '<div id="contentRight" class="contentRight block" style="left: 3px; width:824px; position:relative;">\
					<!--div style="border:1px solid #ccc; padding:30px 15px 70px 15px; margin-bottom: 40px"-->\
				...
				jQNPTemplateHeader.find('#npHeadline').text((!!newspaperJson.headline ? newspaperJson.headline : ''));
				jQNPTemplateHeader.find('#npEdition').text((!!newspaperJson.edition ? newspaperJson.edition : ''));
				jQNPTemplateHeader.find('#npLocation').text((!!newspaperJson.location ? newspaperJson.location : ''));
				...
				if( 'stories' in newspaperJson ){
					...
					for( var i = 0; i < numStories; i++ ){
						...
						//$newspaperView.addWidget(storyHtmlElem, dimension);
		...
		if( npId > 0 ){
			...
			//$newspaperView.cancelNewspaper();
			$('.pageFull, .newspaper-share').detach();
			$('#returnToTop').parent().detach();
		}
	}
    
	$newspaperView.init = function(npid){
		loadNewspaper(npid);

		$('.newspaper-share').click(function(){
			var newspaperHeadline = $('#npHeadline').text();
            TN.sharingHandler.initNPShare(npid, newspaperHeadline);
		});
	};
	
	/*$newspaperView.showPreview = function(){
